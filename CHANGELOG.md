...
- **Protected files are no longer deleted when Pika doesn't have their directory** — if a directory existed in your project but not in the framework, `pika sync` queued the *directory* for deletion and removed it recursively, taking any protected file inside it with it. Protection was consulted when *building* the deletion list but never when *applying* one, so per-file `userProtectedAreas` entries inside such a directory looked protective in both your config and the dry-run, and were silently ignored — the dry-run even listed the files correctly as protected, then listed the parent directory on the next line. Deletions now honour protected areas at both points: the directory is no longer queued when protected content lives beneath it, and a directory deletion that reaches the apply step prunes protected paths instead of removing the tree wholesale. A directory with nothing protected under it is still deleted outright, so nothing that used to be cleaned up is left behind. If you worked around this by protecting such directories with a `/**` glob, that remains correct and needs no change. [#163]

### Added

- **`pika sync` now tells you when Pika has adopted a path you protect** — once the framework starts shipping a file at a path in your `userProtectedAreas`, your copy stops tracking upstream permanently and nothing ever said so; the only way to find out was to audit every protected entry against each new version by hand. Sync now reports those paths, with the steps to take the framework's version if you want it. Reporting only — protected files are still never overwritten and the exit code is unchanged. Shown on `--dry-run` as well. Paths from the framework's own default protected list (`package.json`, `.gitignore`, `README.md` and friends) are summarised as a count rather than listed, since every consumer diverges from those by design. [#163]
...
- **Generic robustness improvements** ported from a downstream consumer:
...
[Search the repository](https://github.com/rithum/pika/search?q=%40since+0.20.0) for @since 0.20.0 to find all type definitions that were added, updated, or removed in this release.
