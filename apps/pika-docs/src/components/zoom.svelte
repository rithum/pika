...
<div class="zoom-wrapper" bind:this={containerRef}>
    ...
    <div class="zoomable" class:with-border={border} on:click={openZoom}>
        <slot />
    </div>
</div>

<dialog bind:this={dialogRef} class="zoom-dialog" on:click={handleDialogClick}>
    {#if isOpen}
        <div class="zoom-content" transition:fade={{ duration: 300, easing: cubicOut }}>
            <figure in:scale={{ duration: 400, start: 0.85, easing: quintOut }} out:scale={{ duration: 300, start: 1, easing: cubicOut }}>
                <img src={zoomedImgSrc} alt={zoomedImgAlt} class="zoomed-image" />
                {#if zoomedImgAlt}
                    <figcaption in:fade={{ duration: 300, delay: 200 }} out:fade={{ duration: 200 }}>
                        {zoomedImgAlt}
                    </figcaption>
                {/if}
            </figure>
...
<style>
    ...
    .zoomable.with-border :global(img) {
        border: 1px solid var(--sl-color-gray-6, #d1d5db);
