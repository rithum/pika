<script lang="ts">
    ...
    let {
        ...
        timezone,
        classes,
        autoCorrectTimeForSameDay = false,
    }: Props = $props();
    ...
    function handleTimeChange(type: 'hour' | 'minute', newValue: string) {
        ...
        if (
            !isTimeValid(newIso, newHour, newMinute) &&
            autoCorrectTimeForSameDay &&
            (minValue || maxValue) &&
            !(minValue && maxValue)
        ) {
...
<Popover.Root>
    <Popover.Trigger class={cn('justify-start text-left font-normal', !value && 'text-muted-foreground')}>
        <div class={cn('relative')}>
            <Button
                variant="outline"
                class={cn('justify-start text-left font-normal', !value && 'text-muted-foreground', classes)}
            >
    ...
    <Popover.Content class="w-auto p-0">
        <Calendar
            type="single"
            bind:value={localDateValue}
            minValue={isoToDateOnlyValue(minValue)}
            maxValue={isoToDateOnlyValue(maxValue)}
        ...
        <div class="p-3 border-t border-border flex flex-col gap-2">
            ...
            {#if minValue}
                <div
                    class="text-xs transition-colors duration-200 {isFlashingRed
                        ? 'text-destructive'
