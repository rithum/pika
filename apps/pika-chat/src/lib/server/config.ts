...
export class AppConfigProxy implements AppConfig {
    ...
    private _platformApiBaseUrl: string | undefined;
    private _oauthUrl: string | undefined;
    private _tokenUrl: string | undefined;
    ...
    private get initConfig(): ConfigType[] {
        return [
            ...
            },
            {
                ...
                setValue: async (_isLocal: boolean, _stage: string, _cache: Cache) => {
                    const result = env.PLATFORM_API_BASE_URL ?? process.env.PLATFORM_API_BASE_URL;
                    if (!result) {
            ...
            },
            {
                name: 'tokenUrl',
                setValue: async (_isLocal: boolean, _stage: string, _cache: Cache) => {
                    const result = env.TOKEN_URL ?? process.env.TOKEN_URL;
                    if (!result) {
    ...
    public get platformApiBaseUrl(): string {
        if (!this._platformApiBaseUrl) throw new Error('App config not initialized');
        return this._platformApiBaseUrl;
    }

    public get oauthUrl(): string {
        if (!this._oauthUrl) throw new Error('App config not initialized');
        return this._oauthUrl;
    }

    public get tokenUrl(): string {
        if (!this._tokenUrl) throw new Error('App config not initialized');
        return this._tokenUrl;
    }

    public get clientId(): string {
        if (!this._clientId) throw new Error('App config not initialized');
