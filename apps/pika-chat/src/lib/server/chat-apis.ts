import type { ChatMessagesResponse, ChatSession, ChatSessionsResponse, ChatUser, ChatUserAddOrUpdateResponse, ChatUserResponse } from '@pika/shared/types/chatbot/chatbot-types';
...
export async function getChatUser(userId: string): Promise<ChatUser | undefined> {
    const response = await invokeApi<ChatUserResponse>({
