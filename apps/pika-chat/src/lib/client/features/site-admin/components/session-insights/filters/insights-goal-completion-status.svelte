<script lang="ts">
    ...
    let valueForList = $derived.by(() => {
        if (!insights?.goalCompletionStatus || insights.goalCompletionStatus.length === 0) {
            return undefined;
        } else {
            return insights.goalCompletionStatus.map((status) => {
                const val = SESSION_INSIGHT_GOAL_COMPLETION_STATUS_VALUES.find((s) => s.value === status);
                if (val) {
                    return val;
                } else {
                    return { value: status, name: status } as NameValueDescTriple<SessionInsightGoalCompletionStatus>;
                }
            });
        }
    });
...
<div class="flex flex-col gap-1">
    <div class="flex items-center gap-2 w-full">
        <PopupHelp popoverClasses="text-xs w-auto p-1">Filter by goal completion status</PopupHelp>
        <SimpleDropdown
            bind:value={
                () => valueForList,
                (val) => {
                    if (val === undefined || val.length === 0) {
                        ...
                        insights = { hasInsights: true, goalCompletionStatus: getValues(val) };
                    } else {
                        if (!insights.hasInsights) {
                            insights.hasInsights = true;
                        }
                        insights.goalCompletionStatus = getValues(val);
                    }
                }
            }
            multiSelect={true}
            ...
            options={SESSION_INSIGHT_GOAL_COMPLETION_STATUS_VALUES}
            dontShowSearchInput={true}
            allowClear={true}
            inputPlaceholder="Filter by goal completion status..."
        />
        <Button
            variant="ghost"
            size="icon"
            ...
            onclick={() => {
                if (insights) {
                    insights.goalCompletionStatus = undefined;
                }
            }}
        >
            <X />
