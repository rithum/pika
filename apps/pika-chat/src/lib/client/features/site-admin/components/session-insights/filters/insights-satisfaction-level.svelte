<script lang="ts">
    ...
    let valueForList = $derived.by(() => {
        if (!insights?.satisfactionLevel || insights.satisfactionLevel.length === 0) {
            return undefined;
        } else {
            return insights.satisfactionLevel.map((status) => {
                const val = SESSION_INSIGHT_SATISFACTION_LEVEL_VALUES.find((s) => s.value === status);
                if (val) {
                    return val;
                } else {
                    return { value: status, name: status } as NameValueDescTriple<SessionInsightSatisfactionLevel>;
                }
            });
        }
    });
...
<div class="flex flex-col gap-1">
    <div class="flex items-center gap-2 w-full">
        <PopupHelp popoverClasses="text-xs w-auto p-1">Filter by satisfaction level</PopupHelp>
        <SimpleDropdown
            bind:value={
                () => valueForList,
                (val) => {
                    if (val === undefined || val.length === 0) {
                        ...
                        insights = { hasInsights: true, satisfactionLevel: getValues(val) };
                    } else {
                        if (!insights.hasInsights) {
                            insights.hasInsights = true;
                        }
                        insights.satisfactionLevel = getValues(val);
                    }
                }
            }
            multiSelect={true}
            ...
            options={SESSION_INSIGHT_SATISFACTION_LEVEL_VALUES}
            dontShowSearchInput={true}
            allowClear={true}
            inputPlaceholder="Filter by satisfaction level..."
        />
        <Button
            variant="ghost"
            size="icon"
            ...
            onclick={() => {
                if (insights) {
                    insights.satisfactionLevel = undefined;
                }
            }}
        >
            <X />
