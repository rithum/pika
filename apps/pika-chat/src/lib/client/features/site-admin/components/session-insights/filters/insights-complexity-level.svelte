<script lang="ts">
    ...
    let valueForList = $derived.by(() => {
        if (!insights?.complexityLevel || insights.complexityLevel.length === 0) {
            return undefined;
        } else {
            return insights.complexityLevel.map((status) => {
                const val = SESSION_INSIGHT_METRICS_COMPLEXITY_LEVEL_VALUES.find((s) => s.value === status);
                if (val) {
                    return val;
                } else {
                    return {
                        value: status,
                        name: status,
                    } as NameValueDescTriple<SessionInsightMetricsComplexityLevel>;
                }
            });
        }
    });
...
<div class="flex flex-col gap-1">
    <div class="flex items-center gap-2 w-full">
        <PopupHelp popoverClasses="text-xs w-auto p-1">Filter by complexity level</PopupHelp>
        <SimpleDropdown
            bind:value={
                () => valueForList,
                (val) => {
                    if (val === undefined || val.length === 0) {
                        ...
                        insights = { hasInsights: true, complexityLevel: getValues(val) };
                    } else {
                        if (!insights.hasInsights) {
                            insights.hasInsights = true;
                        }
                        insights.complexityLevel = getValues(val);
                    }
                }
            }
            multiSelect={true}
            ...
            options={SESSION_INSIGHT_METRICS_COMPLEXITY_LEVEL_VALUES}
            dontShowSearchInput={true}
            allowClear={true}
            inputPlaceholder="Filter by complexity level..."
        />
        <Button
            variant="ghost"
            size="icon"
            ...
            onclick={() => {
                if (insights) {
                    insights.complexityLevel = undefined;
                }
            }}
        >
            <X />
