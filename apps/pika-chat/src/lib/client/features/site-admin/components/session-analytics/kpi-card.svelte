<script lang="ts">
    ...
    let { title, value, subtitle, trend, loading = false, helpText, format = 'number' }: Props = $props();
    ...
    const trendColor = $derived(trend ? (trend.direction === 'up' ? 'text-success' : 'text-destructive') : '');
