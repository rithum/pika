<script lang="ts">
	...
	import type { ChatAppLite, ConverseInvocationMode, SimpleOption, UserType } from 'pika-shared/types/chatbot/chatbot-types';
	...
	const selectedChatApps = $derived.by(() => {
		return chatApps.filter((app) =>
			sessionAnalytics.selectedChatAppIds.includes(app.chatAppId)
		);
	});
	...
	const invocationModeOptions: { value: ConverseInvocationMode | 'undefined'; label: string; description: string }[] = [
		{ value: 'undefined', label: 'User-Initiated (undefined)', description: 'Sessions without invocation mode' },
	...
	<div class="flex flex-wrap gap-4 items-end">
			...
			<div class="flex items-center gap-2">
				<Popover.Root bind:open={dateRangeOpen}>
					<Popover.Trigger id="date-range-filter">
						{#snippet child({ props })}
							<Button {...props} variant="outline" size="sm" class="flex flex-1 items-center justify-between min-w-[240px]">
		...
		<div class="flex flex-col gap-1 min-w-[200px]">
			...
			<Popover.Root>
				<Popover.Trigger>
					{#snippet child({ props })}
						<Button {...props} variant="outline" class="flex items-center justify-between min-w-[200px] h-9">
							<span>
		...
		<div class="flex flex-col gap-1 min-w-[200px]">
			...
			<ToggleGroup.Root
				...
				bind:value={
					...
					(value: string[]) => {
						sessionAnalytics.selectedUserTypes = value as UserType[];
					}
				}
			>
				{#each userTypeOptions as userType}
					<ToggleGroup.Item value={userType.value}>
						{userType.label}
					</ToggleGroup.Item>
				{/each}
			</ToggleGroup.Root>
		...
		<div class="flex flex-col gap-1 min-w-[200px]">
			...
			<Popover.Root>
				<Popover.Trigger>
					{#snippet child({ props })}
						<Button {...props} variant="outline" size="sm" class="flex items-center justify-between min-w-[200px]">
							<span>
