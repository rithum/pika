...
export async function siteFeaturesVitePlugin(): Promise<Plugin> {
    ...
    return {
        ...
        async handleHotUpdate(ctx) {
            if (ctx.file.endsWith(pikaConfigFileName)) {
                await updateSiteFeaturesTsFileFromPikaConfig();
                ctx.server.ws.send({
                    type: 'full-reload',
                    path: '*'
...
export const siteFeatures: SiteFeatures | undefined = ${rawSerialized};
`;
