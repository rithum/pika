<script lang="ts">
    ...
    const wcUrl = '/hello-world-wc.js';
    let loaded = $state(false);
    onMount(async () => {
        if (!customElements.get('hello-world-wc')) {
            await import(/* @vite-ignore */ wcUrl);
            console.log('WC loaded');
            console.log(customElements.get('hello-world-wc'));
            loaded = true;
        }
    });
...
{#if loaded}
    <!-- Pass any attributes you need -->
    <hello-world-wc name="Bruce"></hello-world-wc>
{:else}
    <p>Loading widget…</p>
{/if}
