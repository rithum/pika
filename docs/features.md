...
- **logout**: Menu option (access controlled).
- **fileUpload**: Allowed MIME list.
- **suggestions**: Static or per-chat-app suggestions, with count and randomization controls.
- **promptInputFieldLabel**: Custom input label.
- **uiCustomization**: Toggle UI aspects (e.g., show history in standalone, show user region).
- **userDataOverrides**: Site-wide enablement of the user data override experience.
- **entity**: Enables entity-based access control and UI strings for admin tooling (defines `attributeName` path in user custom data).
- **siteAdmin**: Enables Site Admin interface for override management (access via `pika:site-admin`).
- **contentAdmin**: Enables cross-user read-only viewing for debugging (access via `pika:content-admin`).
...
- **Feature override hierarchy**: Site → Chat App → Admin Override; overrides fully replace lower levels (no merging) and can only be more restrictive than site-level enablement.
...
- **User Data Override**: Authorized users can override `ChatUser.customData` per chat app via a custom UI component with autocomplete helpers; persists until cleared or logout.
- **Entity feature**: Declarative entity-based access using `user.customData[attributeName]`, with admin autocomplete function to manage allow-lists per app.
...
- **Verify Response**: Automatic grading and optional auto-reprompt to improve answers; grades appear in traces.

### 10. Insights and feedback (analytics pipeline)

- **Feedback system**: Collect user/admin/AI-generated feedback; stored in DynamoDB and replicated to OpenSearch.
- **Session insights**: EventBridge-scheduled runner analyzes sessions (goal completion, sentiment, complexity, confidence, issues) and writes JSON to S3; OpenSearch kept in sync via stream processors.
- **OpenSearch indexing**: Sessions enriched with embedded insights and feedback for query and analysis.
...
- **Security notes**: Replace mock auth before production, assign `userType` for all users, and configure chat app `userTypes`/`userRoles` explicitly.
...
- `pika create-app <name>`: Scaffold a new Pika project.
- `pika sync [--dry-run|--diff|--visual-diff|--branch <name>|--debug]`: Safely receive framework updates.
