...
import { RandomNumInlineStack } from '../lib/stacks/random-num-inline-stack';
...
async function main() {
    ...
    let projNameL = 'random-num-inline';
    let projNameKebabCase = 'random-num-inline';
    let projNameTitleCase = 'RandomNumInline';
    let projNameCamel = 'randomNumInline';
    let projNameHuman = 'Random Num Inline';

    // Create the Random Num Inline stack
    new RandomNumInlineStack(app, `${projNameKebabCase}-${stage}`, {
        env,
        stage,
        description: `Random Num Inline service infrastructure - ${stage} stage`,
